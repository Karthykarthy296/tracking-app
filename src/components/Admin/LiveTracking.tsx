...
const LiveTracking = () => {
    ...
    useEffect(() => {
        const unsubscribe = locationService.subscribeToAllBuses((locations) => {
            ...
            // Filtering logic moved inside a function to allow periodic re-filtering
            const filterActiveBuses = (busList: BusLocation[]) => {
                const now = Date.now();
                const ACTIVE_THRESHOLD = 60 * 1000; // 60 seconds

                return busList.filter(bus =>
                    bus.isOnline === true &&
                    (now - bus.updatedAt) < ACTIVE_THRESHOLD
                );
            };

            setActiveBuses(filterActiveBuses(locations));
        });

        // Add a local timer to re-evaluate "active" status every 10 seconds 
        // to remove stale vehicles even if no new data comes in for THEM
        const interval = setInterval(() => {
            setActiveBuses(prev => {
                const now = Date.now();
                const ACTIVE_THRESHOLD = 60 * 1000;
                return prev.filter(bus => (now - bus.updatedAt) < ACTIVE_THRESHOLD);
            });
        }, 10000);
                            ...
                            return (
                                ...
                                >
                                    <div className="flex justify-between items-start mb-3">
                                        ...
                                        <div className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${bus.arrivalStatus === 'arriving' ? 'bg-orange-500/20 text-orange-400' :
            ...
            {selectedBus && (
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-slate-900/90 backdrop-blur-xl border border-slate-700 p-5 rounded-2xl shadow-2xl z-20 min-w-[500px] flex gap-6 items-center animate-up-fade">
                    ...
                    <div className="flex-1">
                        ...
                        <p className={`font-semibold ${selectedBus.arrivalStatus === 'arriving' ? 'text-orange-400' : 'text-emerald-400'
                            }`}>
